...
-- Manav tablosundaki urun adlarini tekrarsiz olarak siralayin

SELECT DISTINCT * FROM manav;

-- Manav tablosundaki urunleri ve her bir urunun toplam satis miktarini listeleyin
...
SELECT urun_adi, SUM(miktar_kg) AS toplam_satis
FROM manav
GROUP BY urun_adi
...
SELECT urun_adi , AVG(satis_fiyati) FROM manav
GROUP BY urun_adi;
...
SELECT kategori, MAX(satis_fiyati) AS  en_yuksek_birim_fiyat FROM manav
GROUP BY kategori;
